...
const MakairaAppProvider: React.FC<MakairaAppProviderProps> = ({
  ...
  const handleMessage = useCallback((event: MessageEvent) => {
    // TODO: This check needs to be more strict.
    if (!event.origin.includes('makaira')) return
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === 'development' && !makairaHmac) {
      console.debug(
        '[Simple-PIM-App] Skipping Auth-Request because app runs in dev-mode and query parameters are undefined'
      )
      setToken(process.env.NEXT_PUBLIC_DEV_TOKEN)
      client.current.setToken(process.env.NEXT_PUBLIC_DEV_TOKEN ?? '')
    } else if (!token) {
      console.debug('[Simple-PIM-App] Request Auth-Token from Makaira-Admin-UI')
  ...
  const handleResponseUserRequest = (data: ResponseUserRequestPayload) => {
    console.debug('[Simple-PIM-App] Received token from Makaira Admin UI.')
